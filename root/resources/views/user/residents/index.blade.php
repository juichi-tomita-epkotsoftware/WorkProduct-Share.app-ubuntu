...
<h1>ShareMenbers</h1>
<a href="{{ route('user.residents.create') }}"
...
<div style="margin-bottom:20px;">
    @foreach (['all' => 'All', 'current' => 'Current', 'former' => 'Old'] as $key => $label)
        <a href="{{ route('user.residents.index', ['filter' => $key]) }}"
...
<form method="GET" action="{{ url('user/residents') }}" style="margin: 10px 0;">
    {{-- 検索時もフィルター状態を維持する --}}
    ...
    @if ($keyword)
        <a href="{{ url('user/residents') }}?filter={{ $filter }}" class="btn btn-outline-secondary">Clear</a>
    @endif
</form>
...
<table class="table table-bordered table-hover" style="font-size: 18px; width:auto;" >
    <tr>
        ...
        <th style="white-space:nowrap;">BirthPlace</th>
    ...
    <tr>
        ...
        <td><a href="{{ route('user.residents.show', $resident) }}">{{ $resident->name }}</a></td>
        <td>{{ $resident->job }}</td>
        ...
        <td>
            ...
            <a href="{{ route('user.residents.edit', $resident) }}">Edit</a>
            <form action="{{ route('user.residents.destroy', $resident) }}" method="POST" style="display:inline">
                @csrf
                @method('DELETE')
